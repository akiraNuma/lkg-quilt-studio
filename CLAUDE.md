...
- **機種に既定値を持たせない**: 機種を取り違えた quilt でも変換は最後まで成功し、実機に出して
  初めて崩れて分かる（5x9 の quilt を 11x6 の Go に出した）。Bridge に繋げば `defaultQuilt` から
  自動で入るので、決まるまでプレビューと変換のボタンを押せなくしてある（`QuiltWorkbench.vue` の `ready`）
...
- **収束面は再生側で動かし、その値をそのまま焼き込む**: 収束面を Δ 動かすと、位置 `position` の
  ...
  実機を見ながら動かした値を「動画全体を変換する」で**絶対値として焼き込む**
  （`auto` のままだと動画の先頭フレームで決め直され、見た絵とずれる）。
  **`span`（立体の強さ）は視点そのものを作り直すので描き直しが要る**
- **調整画面は操作を「すぐ効く」と「描き直しが要る」に分ける**: どのつまみが即座に効いて
  どれが 2 秒待つのかが分からないと詰められない（実際に「何をどう触れるのか分からない」と
  言われた）。絵を左に固定して操作を右の列に置き、描き直しが要る設定を変えたら
  「設定を変えた」と出す（`QuiltWorkbench.vue` の `renderKey` / `stale`）
...
- **変換の入口を CLI と HTTP API に分ける**: 画面から動画を投げて進捗を見たいので API を足したが、
  ...
  **所要時間は走っているジョブの実測（開始時刻と済んだ枚数）から出す。**
